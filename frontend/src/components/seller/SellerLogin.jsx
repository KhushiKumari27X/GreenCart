  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        "https://greencart-backend-85xnosoo6-khushikumari27xs-projects.vercel.app/api/seller/login",
        { email, password },
  ...
  return !isSeller ? (
    ...
    >
      ...
      <div className="flex flex-col gap-5 items-start p-8 py-12 min-w-80 sm:min-w-96 rounded-lg shadow-xl border border-gray-200 bg-white">
        ...
        <button
          type="submit"
          className="bg-primary hover:bg-primary-dull transition text-white w-full py-2 rounded-md cursor-pointer"
        >
          Login
        </button>
